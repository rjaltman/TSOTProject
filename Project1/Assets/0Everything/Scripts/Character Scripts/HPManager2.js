...
var player: GameObject; //used to give xp to player
...
function OnHeadshotHit(damage: int)
{
	if(health > 0 && health - (damage * multiplier) <= 0 && !dead)
    {
    	health = 0;
    	player.SendMessage("HeadshotKill", maxHealth);
    	Kill();
    }
    else if(health > 0)
    {
...
{	
    ...
    {
    	health = 0;
    	player.SendMessage("BulletKill", maxHealth);
    	Kill();
...
function OnKnifeHit(damage:int)
{
	if(health > 0 && health - (damage * multiplier) <= 0 && !dead)
    {
    	health = 0;
    	if(damage>100) //backstab
    	player.SendMessage("BackstabKill", maxHealth);
    	else
    	player.SendMessage("KnifeKill", maxHealth);
    	Kill();
    }
    else if(health > 0)
    {
...
function OnMissileHit(damage: int)
{
    ...
    {
    	health = 0;
    	player.SendMessage("MissileKill", maxHealth);
    	Kill();
    }
    else if(health > 0)
    {
